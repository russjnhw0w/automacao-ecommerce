...
// ===================================================================
// INÍCIO DA FUNÇÃO 'preencherCampos' (VERSÃO 7 - LÓGICA SIMPLIFICADA)
// ===================================================================
function preencherCampos() {
  ...
  // 1. Extrair Código CheckMat (procurando por "CK.")
  let codigo = "";
  for (const line of lines) {
    const match = line.match(/\b(CK\.[0-9A-Z-]+)\b/i);
    if (match) {
      codigo = match[1];
      break;
    }
  ...
  const findSection = (startLabels) => {
    ...
    if (startIndex === -1) return ""; // Seção não encontrada

    // Lista de TODAS as possíveis labels que podem terminar uma seção
    ...
    // Começa a ler a partir da linha seguinte à da label
    for (let i = startIndex + 1; i < lines.length; i++) {
      const currentLine = lines[i];
      const lowerTrimmedLine = currentLine.trim().toLowerCase();

      // Se a linha atual for uma nova label, para a extração
      if (ALL_STOP_LABELS.some(stop => lowerTrimmedLine.startsWith(stop))) {
        break;
      }
      
      // Se for uma linha decorativa, para a extração
